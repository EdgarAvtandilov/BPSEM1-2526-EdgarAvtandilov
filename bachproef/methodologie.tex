...
Voor de beschreven basisconfiguratie moeten volgende commando's handmatig uitgevoerd worden op de routers en switches:
    ...
    \end{minted}
    \end{listing}
Zodra er routes van de Ansible controller naar de nodes zijn en de netwerktoestellen zijn voorzien van een IP-adres waarop ze bereikt kunnen worden en SSH opgezet is kan de netwerkbeheerder het playbook uitvoeren.
...
Zo is er een inventarisbestand waarin alle netwerktoestellen gedefinieerd worden, een unieke map voor zowel groepsvariabelen als toestelvariabelen en tot slot een rollen map waarin specifieke configuratietaken staan, deze 'roles' vormen samen het uit te voeren playbook.
...
Zo duurt de geautomatiseerde configuratie van alle routers in de testopstelling het 12 minuten en 51 seconden, een 70,23\% tijdsbesparing vergeleken met handmatige configuratie.
\subsubsection{Switches}

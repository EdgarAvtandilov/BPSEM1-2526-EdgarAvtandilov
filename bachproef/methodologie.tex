...
In de voorbereidingsfase wordt onderzocht hoe een gemiddelde netwerktopologie van een kmo eruitziet om een realistische en representatieve testomgeving op te zetten voor de proof-of-concept.
...
Specifiek wordt er gekeken naar hoeveel toestellen, welke toestellen, het merk van toestellen en welke configuraties de testomgeving moet hebben.
Om het te bepalen welke netwerktoestellen er nodig zijn wordt er voornamelijk gekeken naar bestaande literatuur om te zien welk merk van netwerktoestellen het grootste marktaandeel heeft, dit vergroot de representativiteit van de testomgeving en de toepasbaarheid van de resultaten. 
...
Daarnaast wordt het netwerk zodanig opgebouwd dat uitbreidingen eenvoudig mogelijk blijven, wat aansluit bij de schaalbaarheidsvereisten van kmo's.
...
Aangezien Ansible verbinding met de nodes maakt via SSH moet dit eerst opgezet worden op alle te beheren toestellen. Bij de eerste SSH-setup worden placeholders gebruikt voor variabelen zoals de hostname en domeinnaam. 
Hierdoor kan de netwerkbeheerder dezelfde commando's op elk toestel gebruiken zonder tijd te verliezen aan het aanpassen van variabelen.
Deze eerste basis SSH setup kan op deze manier gedaan worden, omdat deze tijdelijke SSH configuratie bij de geautomatiseerde configuratie herschreven wordt met de gewenste variabelen.
...
Voor de beschreven basisconfiguratie moeten volgende commando's handmatig uitgevoerd worden op de routers en switches:
    ...
    \begin{listing}[H]
    \caption{Basis interfaceconfiguratie op Cisco IOS switches}
    \label{lst:switch-interface-basisconfig}
    \begin{minted}{text}    
    configure terminal
    interface vlan99
    ip address <ip adres> <subnet mask>
    ...
    Deze stap moet voor elk netwerk dat de netwerkbeheerder wil bereiken uitgevoerd worden.
    \begin{listing}[H]
    ...
    \end{minted}
    \end{listing}
Zodra er routes van de Ansible-controller naar de nodes bestaan, de netwerktoestellen een bereikbaar IP-adres hebben en SSH is opgezet, kan de netwerkbeheerder het playbook uitvoeren.
Hiervoor moet de netwerkbeheerder de 'ansible' map openen en onderstaande commando's uitvoeren. De collections installeren moet maar één keer uitgevoerd worden, dit voor het playbook voor het eerst is uitgevoerd op de Ansible controller. 
Daarna staan de collecties, tot ze verwijderd worden, op de Ansible controller.
    \begin{listing}[H]
    \caption{Collections installeren}
    \label{lst:install-collections}
    \begin{minted}{text}   
        ansible-galaxy collection install -r requirements.yml
    \end{minted}
    \end{listing}
    \begin{listing}[H]
    \caption{Uitvoeren playbook}
    \label{lst:run-playbook}
    \begin{minted}{text}   
...
In de structuur vind men een inventarisbestand waarin alle netwerktoestellen gedefinieerd worden, een unieke map voor zowel groepsvariabelen als toestelvariabelen en tot slot een rollen map waarin specifieke configuratietaken staan.
...
Tijdens de evaluatiefase wordt beoordeeld of de werkelijke resultaten overeenkomen met de verwachtingen:
\begin{itemize}
    \item Is configuratie aan de hand van Ansible \textbf{sneller} dan handmatige configuratie?
    \item Is deze manier van configureren \textbf{reproduceerbaar en consistent}? Met andere woorden, kan men de playbooks meermaals heruitvoeren zonder afwijkingen in de eindconfiguratie?
...
\begin{itemize}
    \item handmatige routerconfiguratie: 10 minuten en 48 seconden
    \item handmatige switchconfiguratie: 9 minuten en 12 seconden
    \item geautomatiseerde routerconfiguratie: 3 minuten en 58 seconden
    \item geautomatiseerde switchconfiguratie: 8 minuten en 49 seconden
\end{itemize}

\subsubsection{Routers}
Zoals de bovenvermelde cijfers tonen, duurt één router handmatig configureren gemiddeld 10 minuten en 48 seconden. Één router aan de hand van Ansible configureren kostte daarentegen gemiddeld 3 minuten en 58 seconden.
Om de CompanyRouter te configureren duurde het gemiddeld 5 minuten en 23 seconden, voor de siterouters was dit gemiddeld 2 minuten en 35 seconden.
Hoewel dit voor de configuratie van één toestel een gemiddelde tijdsbesparing is van 63,27\%, maken de tijdsbesparende kwaliteiten van automatische configuratie zich voornamelijk kenbaar in het configureren van meerdere toestellen.
Met een gemiddelde handmatige configuratieduur van 10 minuten en 48 seconden, zou het configureren van alle routers in de opstelling gemiddeld 43 minuten en 12 seconden in beslag nemen. 
Doordat Ansible toestellen parallel configureert, en niet één voor één zoals een netwerkbeheerder handmatig zou moeten doen, wordt de tijd die nodig is om de routers in de testopstelling te configureren drastisch verminderd. 
De geautomatiseerde configuratie van alle routers in de testopstelling duurt gemiddeld 12 minuten en 48 seconden, een 70,37\% tijdsbesparing vergeleken met handmatige configuratie.
Deze configuratietijd is echter niet in lijn met de verwachte configuratieduur. Het playbook configureert eerst enkel de CompanyRouter gevolgd door de parallelle configuratie van de siterouters. 
De verwachte configuratieduur zou dus de gemiddelde configuratieduur van de CompanyRouter opgeteld met de gemiddelde duur van de siterouters zijn, wat in de virtuele testomgeving niet het geval is. 
De configuratietijd in de virtuele omgeving ziet een toename van 37,76\%, 12 minuten en 48 seconden en niet de verwachte 7 minuten en 58 seconden.
Deze toename in tijd is het gevolg van de netwerktoestellen die gevirtualiseerd zijn en de beschikbare CPU-kracht en RAM onderling moeten verdelen, wat bij fysieke netwerktoestellen niet het geval is. 

\subsubsection{Switches}
Ook bij de switches bespaart configuratie met behulp van Ansible tijd, maar minder significant bij één enkele switch dan bij een router. Om één switch geautomatiseerd te configureren duurt het gemiddeld 4,17\% minder lang.
Echter is er wel een grote tijdsbesparing, net zoals bij routers, merkbaar bij het configureren van meerdere switches. 
De drie uplink switches handmatig configureren zou gemiddeld 27 minuten en 36 seconden in beslag nemen. 
Ansible maakt de configuratie af in 21 minuten en 42 seconden, een tijdsbesparing van 21,38\%.
Ook hier ziet men een grotere tijdsbesparing bij het parallel configureren van meerdere toestellen.
\vspace{1em}

\subsection{Reproduceerbaarheid en consistentie}
Dit onderzoek toont aan dat Ansible zorgt voor betere reproduceerbaarheid en dat configuratie consistent blijft bij het heruitvoeren van het Ansible playbook.
...
Bij het herhalen van de playbooks vergelijken de modules de op Cisco-toestellen aanwezige running configuration met de gewenste configuratie, waarbij enkel de afwijkende configuratie wordt aangepast.
Bestaande configuratie die al overeenkomt met de gewenste configuratie wordt als 'ok' gemarkeerd door Ansible, waarna de volgende stap van het playbook begint.
Enkel afwijkende configuratie wordt dus aangepast en wordt als 'changed' gemarkeerd.
Hierdoor kan een netwerkbeheerder nagaan en volgen welke configuratie daadwerkelijk aangepast werd en daarmee ook bevestigen of gewenste aanpassingen effectief zijn uitgevoerd.
...
Doordat configuraties niet manueel worden ingegeven, wordt het aantal typfouten ook aanzienlijk verminderd.
\vspace{1em}
...
In tegenstelling tot handmatige configuratie is er bovendien steeds een duidelijk overzicht van de gewenste configuratiestatus, wat troubleshooting vereenvoudigt.
\vspace{1em}

Op basis van deze bevindingen kan worden geconcludeerd dat Ansible niet alleen efficiënter is dan de handmatige configuratie, maar ook bijdraagt aan het verminderen van menselijke fouten en het verhogen van de stabiliteit van de netwerkinfrastructuur.
\vspace{1em}